    ...
    <header className="sticky top-0 z-40 bg-white border-b border-slate-100 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            ...
            >
              <img
                src={user.profileImg}
                alt=""
                className="w-8 h-8 rounded-xl object-cover bg-slate-100 flex-shrink-0"
              />
            ...
            {menuOpen && (
              <div className="absolute right-0 mt-2 w-52 bg-white rounded-2xl shadow-xl border border-slate-100 py-2 z-50">
                ...
                >
                  <i className="fas fa-user w-4 text-slate-400 text-xs"></i> My
                  Profile
                </Link>
                <Link
                  ...
                  className="flex items-center gap-2.5 px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 transition-colors"
                >
                  <i className="fas fa-key w-4 text-slate-400 text-xs"></i>{" "}
                  Change Password
                </Link>
          ...
          {/* Mobile menu */}
          <button
            className="md:hidden w-9 h-9 rounded-xl bg-slate-100 flex items-center justify-center text-slate-500"
            onClick={() => setMenuOpen(!menuOpen)}
          >
