...
import {
  get_checkpoints_details,
  update_checkpoint_score,
...
const CheckpointValidate = () => {
  ...
  useEffect(() => {
    dispatch(get_checkpoints_details(id));
  }, [id]);
  ...
  const findContent = () => {
    ...
    try {
      const course = user.course.find(
        (el) => el._id === checkpointDetails.student?.course?.[0]?.course,
      );
      for (const section of course.data) {
        const found = section.superSkills?.find(
          (el) => el._id === checkpointDetails.checkpointId,
        );
  ...
  const handleSubmit = () => {
    dispatch(update_checkpoint_score({ id, value, navigate }));
  };
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <p className="text-slate-500 text-sm mt-0.5">
              {checkpointDetails?.student?.firstName}{" "}
              {checkpointDetails?.student?.lastName} —{" "}
        ...
        <div className="flex gap-1 bg-white border border-slate-100 rounded-2xl p-1.5 mb-6 shadow-sm max-w-xs">
          {[
            { key: "score", label: "Score", icon: "fas fa-star" },
            { key: "details", label: "Details", icon: "fas fa-file-alt" },
          ].map((tab) => (
            <button
              key={tab.key}
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-7 max-w-lg">
            ...
            {checkpointDetails?.student && (
              <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-xl mb-6">
                <img
                  src={checkpointDetails.student.profileImg}
                  alt=""
            ...
            <div className="mb-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm font-bold text-slate-500 w-8 text-right">
                  /10
                </span>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-7">
            ...
            {findContent() ? (
              <div
                ...
                dangerouslySetInnerHTML={{ __html: findContent() }}
              />
            ) : (
