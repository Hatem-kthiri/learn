    ...
    <div className="min-h-screen bg-slate-50 dark:bg-gray-900">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
        <div className="flex items-center gap-3 mb-8">
          <Link to="/checkpoints" className="w-9 h-9 rounded-xl bg-white border border-slate-200 hover:bg-slate-50 flex items-center justify-center text-slate-500 transition-colors dark:bg-gray-800 dark:border-gray-600 dark:hover:bg-gray-700 dark:text-gray-400">
            <i className="fas fa-arrow-left text-sm"></i>
          </Link>
          <div>
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Validate Checkpoint</h1>
            <p className="text-slate-500 text-sm mt-0.5 dark:text-gray-400">
        ...
        {/* Tabs */}
        <div className="flex gap-1 bg-white border border-slate-100 rounded-2xl p-1.5 mb-6 shadow-sm max-w-xs dark:bg-gray-800 dark:border-gray-700">
        ...
        {activeTab === "score" && (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-7 max-w-lg dark:bg-gray-800 dark:border-gray-700">
            <h2 className="font-bold text-slate-900 mb-5 dark:text-white">Assign Score</h2>

            {/* Student info */}
            {checkpointDetails?.student && (
              <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-xl mb-6 dark:bg-gray-900">
                <img src={checkpointDetails.student.profileImg} alt="" className="w-10 h-10 rounded-xl object-cover bg-slate-100 dark:bg-gray-700" />
                <div>
                  <p className="font-bold text-slate-900 dark:text-white">{checkpointDetails.student.firstName} {checkpointDetails.student.lastName}</p>
                  <p className="text-xs text-slate-500 dark:text-gray-400">{checkpointDetails.checkpointName}</p>
                </div>
              </div>
            )}
            ...
            {checkpointDetails?.link && (
              <div className="mb-5">
                <label className="block text-sm font-semibold text-slate-700 mb-2 dark:text-gray-200">Submission Link</label>
            ...
            {/* Score input */}
            <div className="mb-6">
              <label className="block text-sm font-semibold text-slate-700 mb-2 dark:text-gray-200">Score (0–10)</label>
              <div className="flex items-center gap-3">
                <input type="number" value={value} onChange={handleChange} min="0" max="10" placeholder="0"
                  className="w-24 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-2xl font-bold text-center focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all dark:bg-gray-900 dark:border-gray-600" />
                <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden dark:bg-gray-700">
                  <div className="h-full bg-indigo-500 rounded-full transition-all duration-300" style={{ width: `${(value / 10) * 100}%` }}></div>
                </div>
                <span className="text-sm font-bold text-slate-500 w-8 text-right dark:text-gray-400">/10</span>
              </div>
            </div>
        ...
        {activeTab === "details" && (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-7 dark:bg-gray-800 dark:border-gray-700">
            <h2 className="font-bold text-slate-900 mb-5 dark:text-white">Checkpoint Content</h2>
            {findContent() ? (
              <div className="prose max-w-none text-slate-700 text-sm leading-relaxed dark:text-gray-200" dangerouslySetInnerHTML={{ __html: findContent() }} />
            ) : (
