      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">{meetings?.length || 0} scheduled meetings</p>
          </div>
        ...
        {showAdd && (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 mb-6">
            ...
            <div className="flex gap-3">
              <button onClick={() => { setShowAdd(false); setMeetDetails({}); }} className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold py-3 rounded-xl text-sm transition-all">Cancel</button>
            ...
            <div key={i} className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5">
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="flex items-center gap-1.5">
                  <button onClick={() => handleEdit(meeting)} className="w-8 h-8 rounded-lg bg-slate-100 hover:bg-indigo-100 text-slate-500 hover:text-indigo-600 flex items-center justify-center transition-colors">
                    <i className="fas fa-edit text-xs"></i>
                  </button>
                  <button onClick={() => handleDelete(meeting._id)} className="w-8 h-8 rounded-lg bg-slate-100 hover:bg-red-100 text-slate-500 hover:text-red-500 flex items-center justify-center transition-colors">
                    <i className="fas fa-trash-alt text-xs"></i>
                  </button>
                </div>
              </div>
              <h3 className="font-bold text-slate-900 mb-1">{meeting.name}</h3>
              {meeting.date && <p className="text-xs text-slate-500 mb-3"><i className="fas fa-calendar-alt mr-1.5"></i>{new Date(meeting.date).toLocaleString()}</p>}
