...
const Meetings = () => {
  ...
  useEffect(() => {
    if (!userLoading) dispatch(get_meetings(user));
  }, [user, userLoading]);

  // Guilds the logged-in instructor actually belongs to (array of guild names)
  const instructorGuilds = Array.isArray(user?.guild)
    ? user.guild
    : user?.guild
      ? [user.guild]
      : [];
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (r.isConfirmed) dispatch(delete_meeting({ user, id }));
    });
  };

  const handleEdit = (meeting) => {
    // Normalize "time" back into the datetime-local input format (YYYY-MM-DDTHH:mm)
    const timeValue = meeting.time
      ? new Date(meeting.time).toISOString().slice(0, 16)
      : "";
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 mb-6">
              ...
              <div>
                ...
                {instructorGuilds.length === 0 && (
                  <p className="text-xs text-red-500 mt-1">
                    You are not assigned to any guild yet.
                  </p>
                )}
              </div>
              {fields.map((f) => (
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-3 rounded-xl text-sm transition-all inline-flex items-center justify-center gap-2"
              >
              ...
              <div className="flex items-center justify-between">
                ...
                {meeting.link && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold px-4 py-1.5 rounded-lg text-xs transition-colors inline-flex items-center gap-1.5"
                  >
          ...
          {(!meetings || meetings.length === 0) && !showAdd && (
            <div className="col-span-2 text-center py-20 text-slate-400">
              ...
              <button
                ...
                className="mt-4 inline-flex items-center gap-2 bg-indigo-600 text-white font-semibold px-5 py-2.5 rounded-xl text-sm"
              >
