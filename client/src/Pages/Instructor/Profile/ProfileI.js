...
const ProfileI = () => {
  ...
  const handleUpdate = () => {
    ...
    ["address", "firstName", "lastName", "phone"].forEach((k) => {
      if (instructorProfile[k] !== undefined)
        formData.append(k, instructorProfile[k]);
    });
    setLoading(true);
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-7">
          ...
          <div className="flex items-center gap-5 mb-8 pb-8 border-b border-slate-100">
            ...
            <div>
              ...
              <span className="mt-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-violet-100 text-violet-700">
                <i className="fas fa-chalkboard-teacher mr-1 text-xs"></i>{" "}
                Instructor
              </span>
            </div>
          </div>
          ...
          {user.guild && (
            <div className="mb-6 p-4 bg-slate-50 rounded-xl border border-slate-200">
              ...
              <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold bg-indigo-100 text-indigo-700">
                <i className="fas fa-award mr-1.5 text-xs"></i>
                {user.guild}
              </span>
            </div>
          )}
          ...
          <button
            ...
            className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-8 py-3 rounded-xl text-sm transition-all flex items-center gap-2"
          >
            {loading ? (
