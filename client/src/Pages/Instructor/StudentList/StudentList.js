      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">{studentsProgress.length} students in your guild</p>
          </div>
        </div>
              ...
              <tbody className="divide-y divide-slate-50">
                {filtered.map((student, i) => (
                  <tr key={student._id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-4"><img src={student.profileImg} alt="" className="w-9 h-9 rounded-xl object-cover bg-slate-100" /></td>
                    <td className="px-4 py-4 font-semibold text-slate-900">{student.firstName} {student.lastName}</td>
                    <td className="px-4 py-4 text-slate-500 text-xs">{student.course?.[0]?.course?.title}</td>
                    ...
                    <td className="px-4 py-4 min-w-[140px]">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-slate-500 flex-shrink-0">{student.course?.[0]?.learnProgress || 0}%</span>
                      </div>
                    </td>
