      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1 dark:text-gray-400">{workshops?.length || 0} workshops created</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          {workshops?.map?.((workshop, i) => (
            <div key={i} className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden group hover:shadow-md transition-shadow dark:bg-gray-800 dark:border-gray-700">
              {workshop.image ? (
              ...
              <div className="p-5">
                <div className="flex items-center justify-between mb-1">
                  <h3 className="font-bold text-slate-900 dark:text-white">{workshop.name}</h3>
                  {workshop.guild && (
                ...
                <p className="text-slate-500 text-sm line-clamp-2 mb-4 dark:text-gray-400">{workshop.description}</p>
                {workshop.date && <p className="text-xs text-slate-400 mb-4 dark:text-gray-500"><i className="fas fa-calendar-alt mr-1.5"></i>{new Date(workshop.date).toLocaleDateString()}</p>}
                {workshop.link && (
                ...
                <div className="flex gap-2">
                  <button onClick={() => handleEditShow(workshop._id)} className="flex-1 flex items-center justify-center gap-1.5 bg-slate-100 hover:bg-slate-200 text-slate-600 font-semibold py-2 rounded-xl text-xs transition-colors dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-gray-300">
