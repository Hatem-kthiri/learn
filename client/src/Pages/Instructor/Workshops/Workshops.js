...
const Workshops = () => {
  ...
  useEffect(() => {
    if (!userLoading) dispatch(get_workshops(user._id));
  }, [userLoading]);
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (r.isConfirmed) dispatch(delete_workshop({ user, id }));
    });
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-gray-900">
              ...
              <div className="p-5">
                ...
                {workshop.link && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="block text-center bg-indigo-50 hover:bg-indigo-100 text-indigo-600 font-semibold py-2 rounded-xl text-sm transition-colors mb-3"
                  >
                    <i className="fas fa-external-link-alt mr-1.5"></i> Join
                    Workshop
                  </a>
                )}
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-1.5 bg-red-50 hover:bg-red-100 text-red-500 font-semibold py-2 rounded-xl text-xs transition-colors"
                  >
      ...
      <AddWorkShop
        config={config}
        closeModal={() => setConfig({ show: false })}
        user={user}
      />
      <EditWorkShop
        editConfig={editConfig}
        closeModal={() => setEditConfig({ show: false })}
