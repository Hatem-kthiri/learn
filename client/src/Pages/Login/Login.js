...
const Login = () => {
  ...
  const { loading, isAuth, role, error } = useSelector((state) => state.LoginReducer);
  ...
  useEffect(() => {
    if (isAuth) {
      setTimeout(() => {
        if (role == 1) navigate("/dashboard-instructor");
        else if (role == 2) navigate("/dashboard-student");
      }, 1000);
    }
  }, [isAuth]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#1e1b4b] via-[#312e81] to-[#4338ca] flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-indigo-300 mt-1 text-sm">Your learning journey starts here</p>
        </div>

        <div className="bg-white rounded-3xl shadow-2xl p-8">
          <h2 className="text-2xl font-bold text-slate-900 mb-1">Welcome back</h2>
          <p className="text-slate-500 text-sm mb-7">Sign in to continue learning</p>

          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-2">Email Address</label>
              <div className="relative">
                <input type="email" name="email" required maxLength={64} placeholder="your@email.com"
            ...
            <div>
              ...
              <div className="relative">
                <input type={showPassword ? "text" : "password"} name="password" required maxLength={64} placeholder="••••••••"
                  onChange={handleChange}
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-11 pr-11 py-3 text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all" />
            ...
            <button type="submit"
              ...
              {loading ? <ClipLoader color="#ffffff" size={20} /> : <><i className="fas fa-sign-in-alt"></i> Sign In</>}
            </button>
          </form>
        </div>
