...
import { add_checkpoint, get_checkpoint, get_learning_schedule, update_checkpoint } from "../../../redux/actions/StudentAction";
...
const Checkpoint = () => {
  ...
  useEffect(() => {
    ...
    else if (checkpointList.length > 0) dispatch(get_checkpoint({ user, id: checkpointList[0]._id }));
  }, [user, checkpointList]);
  ...
  useEffect(() => {
    if (learningSchedule) setCheckpointListFromLearn(extractFromLearn(learningSchedule));
  }, [learningSchedule]);
  ...
  useEffect(() => {
    if (user.course && checkpointList.length > 0) {
      ...
      for (const section of allData) {
        ...
        if (found) { setCheckpointContent(found); break; }
      }
    }
  }, [checkpointList, id]);

  const isUnlocked = (cpId) => checkpointListFromLearn.some((el) => el._id === cpId);
  const isSubmitted = () => checkpointSubmited && checkpointSubmited._id === (id || checkpointList[0]?._id);

  const handleSubmit = () => {
    const activeId = id || checkpointList[0]?._id;
    if (isSubmitted()) {
      dispatch(update_checkpoint({ user, id: activeId, link }));
    } else {
      const name = checkpointContent?.title || "Checkpoint";
      dispatch(add_checkpoint({ user, id: activeId, link, name }));
    }
  };
    ...
    <div className={`min-h-screen flex flex-col ${night_mode ? "bg-gray-900" : "bg-slate-50"}`}>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        <div className="mb-8">
          ...
          <p className="text-slate-500 text-sm mt-1">{checkpointListFromLearn.length} unlocked checkpoints</p>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Sidebar list */}
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 h-fit">
            <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3 px-1">All Checkpoints</p>
            <div className="space-y-1">
              {checkpointList.map((cp, i) => {
                const unlocked = isUnlocked(cp._id);
                const active = (id || checkpointList[0]?._id) === cp._id;
                return (
                  <Link key={i} to={unlocked ? `/checkpoint/${cp._id}` : "#"}
                    ...
                    <i className={`fas ${unlocked ? "fa-flag-checkered" : "fa-lock"} text-xs flex-shrink-0`}></i>
                    <span className="font-medium truncate">{cp.title}</span>
                    {unlocked && !active && <i className="fas fa-chevron-right text-xs ml-auto opacity-40"></i>}
                  </Link>
                );
              })}
            ...
            {checkpointContent ? (
              <>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
                  <h2 className="font-bold text-slate-900 text-lg mb-4">{checkpointContent.title}</h2>
                  {checkpointContent.skillsData?.[0]?.content && (
                    <div className="prose max-w-none text-slate-700 text-sm leading-relaxed"
                      dangerouslySetInnerHTML={{ __html: checkpointContent.skillsData[0].content }} />
                  )}
                </div>
                  ...
                  <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
                    ...
                    {checkpointSubmited?.link && (
                      <div className="mb-4 p-3 bg-emerald-50 border border-emerald-200 rounded-xl">
                        <p className="text-xs font-semibold text-emerald-700 mb-1">Current submission</p>
                        <a href={checkpointSubmited.link} target="_blank" rel="noreferrer" className="text-emerald-600 text-sm hover:underline break-all">{checkpointSubmited.link}</a>
                        {checkpointSubmited.score && <p className="text-xs text-emerald-600 mt-1 font-bold">Score: {checkpointSubmited.score}/10</p>}
                      </div>
                    )}
                    <div className="flex gap-3">
                      <input type="url" placeholder="https://github.com/your-repo" value={link} onChange={(e) => setLink(e.target.value)}
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-16 text-center text-slate-400">
                ...
                <p className="font-semibold">Select a checkpoint to get started</p>
              </div>
            )}
          </div>
        </div>
