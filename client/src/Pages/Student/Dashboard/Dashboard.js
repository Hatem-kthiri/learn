...
const DashboardStudent = () => {
  ...
  useEffect(() => {
    dispatch(get_all_quiz(user));
    dispatch(get_all_checkpoint(user));
  }, [user]);
  ...
  const ResumeCourse = () => {
    ...
    for (let i = learningSchedule.learning.length - 1; i >= 0; i--) {
      for (
        let j = learningSchedule.learning[i].details.length - 1;
        j >= 0;
        j--
      ) {
        if (learningSchedule.learning[i].details[j].open) {
          lastOpenSkill = learningSchedule.learning[i].details[j];
          superSkillsId = learningSchedule.learning[i]._id;
          break;
        }
      }
  ...
  return (
    <div
      className={`min-h-screen flex flex-col ${dm ? "bg-gray-900" : "bg-slate-50"}`}
    >
      <HeaderS />
      <div className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 py-8">
        <div className="mb-8">
          <h1
            className={`text-2xl font-bold ${dm ? "text-white" : "text-slate-900"}`}
          >
            My Dashboard
          </h1>
          <p
            className={`text-sm mt-1 ${dm ? "text-gray-400" : "text-slate-500"}`}
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
          ...
          <div className="lg:col-span-2 bg-gradient-to-br from-indigo-600 to-violet-600 rounded-2xl p-7 text-white shadow-lg shadow-indigo-200/40">
            <div className="flex items-start justify-between mb-6">
              <div>
                ...
                <p className="text-indigo-200 text-sm mt-1">
                  Total Score:{" "}
                  <span className="text-white font-bold">{calcScore()}</span>
                </p>
              </div>
            ...
            {(resumeLink || user.course) && (
              <Link
                to={
                  resumeLink
                    ? `/course/${resumeLink[0]}/${resumeLink[1]}`
                    : `/course/${user.course?.[0]?.courseId?.[0]}/${user.course?.[0]?.courseId?.[1]}`
                }
          ...
          {/* Courses panel */}
          <div
            className={`rounded-2xl p-5 ${dm ? "bg-gray-800 border border-gray-700" : "bg-white border border-slate-100 shadow-sm"}`}
          >
            <h2
              className={`font-bold mb-4 flex items-center justify-between ${dm ? "text-white" : "text-slate-900"}`}
            >
              My Courses
              <Link
                to="/courses"
            ...
            {userLoading ? (
              <div className="space-y-3">
                {[1, 2].map((i) => (
                  <div
                    key={i}
                    className={`h-16 rounded-xl animate-pulse ${dm ? "bg-gray-700" : "bg-slate-100"}`}
                  ></div>
                ))}
              </div>
            ) : (
              user.course?.map((enrollment, i) => (
                <div
                  key={i}
                  className={`flex gap-3 mb-3 last:mb-0 p-3 rounded-xl ${dm ? "bg-gray-700" : "bg-slate-50"}`}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p
                      className={`font-semibold text-sm truncate ${dm ? "text-white" : "text-slate-900"}`}
                    >
                      {enrollment.course?.title}
                    </p>
                    <p
                      className={`text-xs mt-0.5 line-clamp-1 ${dm ? "text-gray-400" : "text-slate-400"}`}
                    >
