...
const Profile = () => {
  ...
  const handleChange = (e) => {
    ...
    if (name === "githubLink") { updated[0] = { githublink: value }; setStudentProfile({ ...studentProfile, socialMediaLinks: updated }); }
    else if (name === "linkedinLink") { updated[1] = { linkedinLink: value }; setStudentProfile({ ...studentProfile, socialMediaLinks: updated }); }
  ...
  const handleUpdate = () => {
    ...
    if (studentProfile.socialMediaLinks) formData.append("socialMediaLinks", JSON.stringify(studentProfile.socialMediaLinks));
    setLoading(true);
  ...
  const fields = [
    ...
    { name: "githubLink", label: "GitHub URL", type: "url", val: user.socialMediaLinks?.[0]?.githublink },
    { name: "linkedinLink", label: "LinkedIn URL", type: "url", val: user.socialMediaLinks?.[1]?.linkedinLink },
  ];
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-7">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-6">
            {fields.map((f) => (
              <div key={f.name}>
                ...
                <input type={f.type} name={f.name} defaultValue={f.val || ""}
          ...
          {/* Course info */}
          {user.course?.map((c, i) => (
            <div key={i} className="mb-5 p-4 bg-slate-50 rounded-xl border border-slate-200">
              <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3">Course Info</p>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
                <div><p className="text-xs text-slate-400">Course</p><p className="font-semibold text-slate-900 mt-0.5">{c.course?.title}</p></div>
                <div><p className="text-xs text-slate-400">Guild</p><p className="font-semibold text-slate-900 mt-0.5">{c.guild}</p></div>
                <div><p className="text-xs text-slate-400">Progress</p><p className="font-semibold text-indigo-600 mt-0.5">{c.learnProgress || 0}%</p></div>
                <div><p className="text-xs text-slate-400">Score</p><p className="font-semibold text-emerald-600 mt-0.5">{c.learnScore || 0}</p></div>
              </div>
            </div>
          ))}

          <button onClick={handleUpdate} className="w-full sm:w-auto bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-8 py-3 rounded-xl text-sm transition-all flex items-center justify-center gap-2">
